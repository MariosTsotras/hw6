...
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next() 
    {
        ...
        if( this->m_ == this->numProbes_) { //means we've probed back to where we started
            return this->npos; 
        }
...
{
    if (((double) (capacity_)) / table_.size() >= resizeAlpha_) {
        resize();
    }
    HashItem* insertItem = new HashItem(p);

    HASH_INDEX_T probedIndex = this->probe(insertItem->item.first);
    if (probedIndex == npos) {
        delete insertItem;
        throw std::logic_error("No available spaces to probe");
    } else {
        if (table_[probedIndex] == nullptr) {
            capacity_ += 1; //increase capacity if we are inserting a new item
            table_[probedIndex] = insertItem;
            return;
        } else {
            delete table_[probedIndex];
            table_[probedIndex] = insertItem;
            return;
        }
    }
...
{
    mIndex_ += 1;
    if (mIndex_ >= 28) { //capacities size
        throw std::logic_error("No more CAPACITIES");
    }
    std::vector<ItemType> tempStorage;

    for (size_t i = 0; i < table_.size(); i++) { //need to go through every index, if nullptr, do nothing, if deleted, delete for good, if not deleted, rehash into new 
        if (table_[i] == nullptr) {
            continue;
        } else if (table_[i]->deleted == true) {
            delete table_[i];
        } else {
            tempStorage.push_back(table_[i]->item);
            delete table_[i];
        }
    }
